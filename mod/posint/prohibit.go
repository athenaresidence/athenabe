...
func GetProhibitedItems(Pesan model.WAMessage, db *mongo.Database) (reply string) {
	...
	listprob, err := mgdb.GetAllDoc[[]Item](db, "prohibited_items", filter)
	if err != nil {
...
func GetCountryFromMessage(message string, db *mongo.Database) (country string, err error) {
	...
	countries, err := mgdb.GetAllDistinctDoc(db, bson.M{}, "Destination", "prohibited_items")
	if err != nil {
		return "", err
	}
