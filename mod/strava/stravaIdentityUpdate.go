...
func StravaIdentityUpdateHandler(Profile model.Profile, Pesan model.WAMessage, db *mongo.Database) string {
	...
	data, err := mgdb.GetOneDoc[StravaIdentity](db, col, bson.M{"phone_number": Pesan.Phone_number})
	if err != nil {
	...
	c.OnScraped(func(r *colly.Response) {
		...
		if data.AthleteId == stravaIdentity.AthleteId {
			...
			_, err := mgdb.UpdateOneDoc(db, col, bson.M{"athlete_id": stravaIdentity.AthleteId}, bson.M{"$set": updateData})
			if err != nil {
