...
func TaskListSave(Pesan model.WAMessage, db *mongo.Database) (reply string) {
	id, _ := GetIDandTask(Pesan.Message)
	idp, err := primitive.ObjectIDFromHex(id)
	if err != nil {
		return "gagal mendapatkan id laporan kak"
	}
	taskall, err := mgdb.GetAllDoc[[]TaskList](db, "tasklist", bson.M{"laporanid": idp})
	if err != nil {
		return "Data task tidak ditemukan kak"
	}
	conf, err := mgdb.GetOneDoc[Config](db, "config", bson.M{"phonenumber": "628999710040"})
	if err != nil {
		return "Wah kak " + Pesan.Alias_name + " mohon maaf ada kesalahan dalam pengambilan config di database " + err.Error()
	}
	statuscode, httpresp, err := jsonapi.PostStructWithToken[model.Response]("secret", conf.DomyikadoSecret, taskall, conf.DomyikadoTaskListURL)
	if err != nil {
	...
	_, err = mgdb.DeleteManyDocs(db, "tasklist", bson.M{"laporanid": idp})
	if err != nil {
...
func TaskListAppend(Pesan model.WAMessage, db *mongo.Database) (reply string) {
	...
	_, err = mgdb.InsertOneDoc(db, "tasklist", dt)
	if err != nil {
...
func TaskListReset(Pesan model.WAMessage, db *mongo.Database) (reply string) {
	id, _ := GetIDandTask(Pesan.Message)
	idp, err := primitive.ObjectIDFromHex(id)
	if err != nil {
		return "gagal mendapatkan id laporan kak"
