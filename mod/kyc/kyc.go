...
func CekSelfiePulang(Pesan model.WAMessage, db *mongo.Database) (reply string) {
	if Pesan.Filedata == "" {
	...
	conf, err := mgdb.GetOneDoc[Config](db, "config", bson.M{"phonenumber": "62895601060000"})
	if err != nil {
	...
	statuscode, httpresp, err := jsonapi.PostStructWithToken[model.Response]("secret", conf.DomyikadoSecret, datapresensi, conf.DomyikadoPresensiURL)
	if err != nil {
...
func CekKTP(Profile model.Profile, Pesan model.WAMessage, db *mongo.Database) (reply string) {
	if Pesan.Filedata == "" {
		return "Kirim pap nya dulu dong kak.. " + Pesan.Alias_name
	}
	dt := KTPProps{
	...
	statuscode, faceinfo, err := jsonapi.PostStructWithToken[FaceInfo]("secret", conf.LeaflySecret, dt, conf.LeaflyURLKTP)
	if err != nil {
	...
	_, err = mgdb.InsertOneDoc(db, "ktp", faceinfo)
	if err != nil {
		return "Wah kak " + Pesan.Alias_name + " mohon maaf ada kesalahan input ke database " + err.Error()
	}
