...
func StravaActivityHandler(Profile model.Profile, Pesan model.WAMessage, db *mongo.Database) string {
...
func scrapeStravaActivity(db *mongo.Database, url, profilePhone, phone, alias string) string {
	...
	c.OnScraped(func(r *colly.Response) {
		...
		data, err := mgdb.GetOneDoc[StravaActivity](db, col, bson.M{"activity_id": stravaActivity.ActivityId})
		if err != nil && err != mongo.ErrNoDocuments {
			reply += "\n\nError fetching data from MongoDB: " + err.Error()
			return
		}
		...
		if !found {
			...
			_, err = mgdb.InsertOneDoc(db, col, stravaActivity)
			if err != nil {
