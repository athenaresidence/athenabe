...
func PanduanDosen(message model.WAMessage, db *mongo.Database) string {
...
func LoginSiakad(message model.WAMessage, db *mongo.Database) string {
...
func ApproveBAP(message model.WAMessage, db *mongo.Database) string {
	email := extractEmail(message.Message)
	if email == "user@email.com" {
...
func CekApprovalBAP(message model.WAMessage, db *mongo.Database) string {
...
func CetakBAP(message model.WAMessage, db *mongo.Database) string {
...
func ApproveBimbingan(message model.WAMessage, db *mongo.Database) string {
	// Extract information from the message
...
func ApproveBimbinganbyPoin(message model.WAMessage, db *mongo.Database) string {
	// Extract information from the message
	nim, topik := extractNimandTopik(message.Message)
	if nim == "" || topik == "" {
		return "Pesan tidak sesuai format. Gunakan format 'approve bimbingan nim [nim] topik [topik]'"
