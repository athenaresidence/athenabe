...
func GetRekapPendaftaranUsers(db *mongo.Database) (rkp RekapitulasiUser, err error) {
	...
	_, err = mgdb.InsertManyDocs[User](db, "lmsusers", users)
	if err != nil {
	...
	count1, err := mgdb.GetCountDoc(db, "lmsusers", filter)
	if err != nil {
		return
	}
	filter = bson.M{
	...
	count2, err := mgdb.GetCountDoc(db, "lmsusers", filter)
	if err != nil {
		return
	}
	filter = bson.M{
	...
	count3, err := mgdb.GetCountDoc(db, "lmsusers", filter)
	if err != nil {
		return
	}
	filter = bson.M{
	...
	count4, err := mgdb.GetCountDoc(db, "lmsusers", filter)
	if err != nil {
		return
	}
	count5, err := mgdb.GetCountDoc(db, "lmsusers", bson.M{"roles": "Sebagai Pengguna LMS"})
	if err != nil {
		return
	}
	// 1. Belum Lengkap
	...
	err = mgdb.DropCollection(db, "lmsusers")
	if err != nil {
		return
	}
	return
