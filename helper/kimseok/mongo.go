...
func SetConnection() *mongo.Database {
	var DBmongoinfo = mgdb.DBInfo{
		DBString: os.Getenv("MONGOSTRING"),
		DBName:   "AI",
	}
	conn, _ := mgdb.MongoConnect(DBmongoinfo)
	return conn
}
...
func IsPasswordValid(mongoconn *mongo.Database, userdata User) bool {
	filter := bson.M{
		"$or": []bson.M{
...
// Get User
func FindUserByUsername(conn *mongo.Database, username string) (*User, error) {
	var user User
...
func FindAllUsers(conn *mongo.Database) ([]User, error) {
	var users []User
	collection := conn.Collection("users")
...
func QueriesDataRegexp(db *mongo.Database, ctx context.Context, queries string) (dest Datasets, err error) {
...
func QueriesSecret(db *mongo.Database, ctx context.Context, secret string) (dest Secrets, err error) {
...
func QueriesALL(db *mongo.Database, ctx context.Context) (dest []Datasets, err error) {
	filter := bson.M{}
