...
func HandlePomodoroReport(Profile model.Profile, Pesan model.WAMessage, db *mongo.Database) string {
	...
	_, err = mgdb.InsertOneDoc(db, "pomokit", report)
	if err != nil {
...
func GetUserData(Profile model.Profile, Pesan model.WAMessage, db *mongo.Database) (daftar.Userdomyikado, error) {
	...
	statusCode, allUsers, err := jsonapi.GetStructWithToken[[]daftar.Userdomyikado]("login", Profile.Token, conf.DomyikadoAllUserURL)
...
func getPublicKey(db *mongo.Database) (string, error) {
	conf, err := mgdb.GetOneDoc[Config](db, "config", bson.M{"publickeypomokit": bson.M{"$exists": true}})
	if err != nil {
...
func HandlePomodoroStart(Profile model.Profile, Pesan model.WAMessage, db *mongo.Database) string {
