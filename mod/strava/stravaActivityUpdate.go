...
func StravaActivityUpdateIfEmptyDataHandler(Profile model.Profile, Pesan model.WAMessage, db *mongo.Database) string {
...
func scrapeStravaActivityUpdate(db *mongo.Database, url, profilePhone, phone, alias string) string {
	...
	Idata, err := mgdb.GetOneDoc[StravaIdentity](db, "strava_identity", bson.M{"phone_number": phone})
	if err != nil {
	...
	// cek apakah akun strava sudah terdaftar di database
	data, err := mgdb.GetOneDoc[StravaActivity](db, col, filter)
	if err != nil {
		if err == mongo.ErrNoDocuments {
		...
		if data.Distance == "" && data.MovingTime == "" && data.ActivityId == stravaActivity.ActivityId {
			if data.Status == "Invalid" {
				...
				if distanceFloat < 3 {
					...
					_, err := mgdb.UpdateOneDoc(db, col, bson.M{"activity_id": stravaActivity.ActivityId}, bson.M{"$set": updateData})
					if err != nil {
