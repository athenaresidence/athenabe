...
func ArsipGambar(Pesan model.WAMessage, db *mongo.Database) (reply string) {
	if Pesan.Filedata == "" {
	...
	statuscode, faceinfo, err := jsonapi.PostStructWithToken[FaceInfo]("secret", conf.LeaflySecret, dt, conf.LeaflyURLLMSDesaGambar)
	if err != nil {
...
func ArsipFile(Pesan model.WAMessage, db *mongo.Database) (reply string) {
	if Pesan.Filedata == "" {
		return "Kirim nya dengan gambar nya dulu dong kak.. " + Pesan.Alias_name
	}
	dt := FaceDetect{
	...
	statuscode, faceinfo, err := jsonapi.PostStructWithToken[FaceInfo]("secret", conf.LeaflySecret, dt, conf.LeaflyURLLMSDesaFile)
	if err != nil {
		return "Wah kak " + Pesan.Alias_name + " mohon maaf ada kesalahan pemanggilan API leafly: " + err.Error()
	}
	if statuscode != http.StatusOK {
