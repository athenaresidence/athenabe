...
func GetNamaTeamFromPesan(Pesan model.WAMessage, db *mongo.Database) (team string, helpdeskslist []string, err error) {
	msg := strings.ReplaceAll(Pesan.Message, "bantuan", "")
	msg = strings.ReplaceAll(msg, "operator", "")
	msg = strings.TrimSpace(msg)
	helpdesks, err := mgdb.GetAllDistinctDoc(db, bson.M{}, "team", "helpdesk")
	if err != nil {
		return
	}
...
func GetScopeFromTeam(Pesan model.WAMessage, team string, db *mongo.Database) (scope string, scopeslist []string, err error) {
	filter := bson.M{
		"team": team,
	}
	scopes, err := mgdb.GetAllDistinctDoc(db, filter, "scope", "helpdesk")
	if err != nil {
		return
	}
	//mendapatkan keyword masuk ke team yang mana
...
func GetOperatorFromScopeandTeam(scope, team string, db *mongo.Database) (operator Helpdesk, err error) {
	...
	operator, err = mgdb.GetOneLowestDoc[Helpdesk](db, "helpdesk", filter, "jumlahantrian")
	if err != nil {
		return
	}
	...
	_, err = mgdb.ReplaceOneDoc(db, "helpdesk", filter, operator)
	if err != nil {
		return
	}
	return
...
// handling key word
func StartHelpdesk(Pesan model.WAMessage, db *mongo.Database) (reply string) {
	...
	_, err = mgdb.InsertOneDoc(db, "helpdeskuser", user)
	if err != nil {
		return err.Error()
	}
...
// handling key word
func EndHelpdesk(Profile model.Profile, Pesan model.WAMessage, db *mongo.Database) (reply string) {
	...
	helpdeskuser, err := mgdb.GetOneLatestDoc[User](db, "helpdeskuser", bson.M{"_id": objectID, "operator.phonenumbers": Pesan.Phone_number})
	if err != nil {
		reply = err.Error()
		return
	}
	...
	_, err = mgdb.ReplaceOneDoc(db, "helpdeskuser", bson.M{"_id": objectID}, helpdeskuser)
	if err != nil {
		reply = err.Error()
		return
	}
	...
	_, err = mgdb.ReplaceOneDoc(db, "helpdesk", filter, op)
	if err != nil {
		reply = err.Error()
		return
	}
...
// handling key word
func FeedbackHelpdesk(Profile model.Profile, Pesan model.WAMessage, db *mongo.Database) (reply string) {
	msgs := strings.Split(Pesan.Message, "|")
	id := msgs[0]
	// Mengonversi id string ke primitive.ObjectID
	objectID, err := primitive.ObjectIDFromHex(id)
	...
	helpdeskuser, err := mgdb.GetOneLatestDoc[User](db, "helpdeskuser", bson.M{"_id": objectID, "phonenumbers": Pesan.Phone_number})
	if err != nil {
		reply = err.Error()
		return
	}
...
func PenugasanOperator(Profile model.Profile, Pesan model.WAMessage, db *mongo.Database) (reply string, err error) {
	//check apakah tiket dari user sudah di tutup atau belum
	user, err := mgdb.GetOneLatestDoc[User](db, "helpdeskuser", bson.M{"phonenumbers": Pesan.Phone_number})
	if err != nil {
		if err == mongo.ErrNoDocuments {
			//check apakah dia operator yang belum tutup tiketnya
			user, err = mgdb.GetOneLatestDoc[User](db, "helpdeskuser", bson.M{"terlayani": bson.M{"$exists": false}, "operator.phonenumbers": Pesan.Phone_number})
			if err != nil {
				if err == mongo.ErrNoDocuments {
					err = nil
					reply = ""
	...
	if !user.Terlayani {
		...
		_, err = mgdb.ReplaceOneDoc(db, "helpdeskuser", bson.M{"_id": user.ID}, user)
		if err != nil {
			return
		}
